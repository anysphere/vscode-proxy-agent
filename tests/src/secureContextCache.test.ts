...
import { createTlsPatch, resetCaches, secureContextCreateStats, LogLevel, ProxyAgentParams, SecureContextOptionsPatch } from '../../src/index';
...
describe('SecureContext cache', function () {
	...
	it('times and counts only actual builds (cache hits excluded)', function () {
		const { createSecureContext } = createTlsPatch(testParams(), tls);
		const certs = [caCert];
		const before = secureContextCreateStats();

		createSecureContext(connectionOptions(certs)); // Cacheable miss: builds a fresh context.
		createSecureContext(connectionOptions(certs)); // Cacheable hit: reuses, so no build is timed.
		createSecureContext({}); // Not cacheable: builds a fresh context.

		const after = secureContextCreateStats();
		assert.strictEqual(after.count - before.count, 2); // miss + uncacheable built; the hit did not
		assert.ok(after.totalMs >= before.totalMs); // build time only accumulates
	});
